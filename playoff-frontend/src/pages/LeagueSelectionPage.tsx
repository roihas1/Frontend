...
const LeaguesSelectionPage: React.FC = () => {
  ...
  const handleJoinLeague = async () => {
    ...
    try {
      ...
      showError(
        error ? `${error.response.data.message}` : `Failed to join to a league.`
      );
    }
  };
  ...
  const handleLeaveLeague=  async (league: League)=>{
    try{
        ...
        setPrivateLeagues((prevState)=> prevState?.filter((prev)=>prev.id !== league?.id))
      ...
      >
        <DialogContent sx={{padding:0}}>
          ...
          <div className="p-6 flex flex-col items-center bg-white space-y-4">
            ...
            {selectedLeague?.name !== "Overall" && selectedLeague?.admin?.id !== currentUser?.id && (
              <button
                className=" w-1/2 text-xl bg-colors-nba-yellow hover:opacity-80 p-2 rounded-lg text-black shadow-md font-semibold"
